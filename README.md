...
**EMD → MRC → restore orientation → (HRTEM) rotate + crop → PNG + SVG with a bottom-right scale bar.**

- STEM and HRTEM (`Camera Ceta`) images; EDS cubes/spectra are skipped.
...
Ask the agent, e.g. *"Use TEM_EMD on this folder; rotate HRTEM 2° counter-clockwise."*
